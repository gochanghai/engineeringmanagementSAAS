  ...
  <div id="e_space">
    ...
    <div class="tabelData">
      <el-table :data="list">
        ...
        <el-table-column prop="harmful" label="危险有害元素"></el-table-column>
...
<script>
import spaceEdit from "../module/e_spaceedit";
...
export default {
  ...
  methods: {
    edit(flag, id) {
      if (flag == "add") {
        this.titleEdit = "添加有限空间基本信息";
      } else {
        this.titleEdit = "修改有限空间基本信息";
      }
      this.isVisible = true;
      this.updateId = id;
    },
    ...
    getListData() {
      let _this = this;
      specialInfoService.listLtdSpace(res => {
        console.log(res);
        if (1 == res.code) {
          _this.list = res.list;
    ...
    del(id) {
      this.$confirm("确认删除吗?", "提示", {
        ...
        .then(() => {
          specialInfoService.deleteLtdSpaceById(id, res => {
            if (1 == res.code) {
              this.$message({
                type: "success",
                message: "操作成功!"
...
<style>
#e_space .search /deep/.el-input__inner {
  border: 1px solid #ccc !important;
  border-radius: 0;
  border-right: none;
}

#e_space .search /deep/.el-button {
