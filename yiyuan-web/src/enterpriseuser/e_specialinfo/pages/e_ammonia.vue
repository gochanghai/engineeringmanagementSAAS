...
<script>
import ammoniaEdit from "../module/e_ammoniaedit.vue";
...
export default {
  data() {
    return {
      ...
      updateId: null,
      fileType: "涉氢操作",
      uploadAction: "/api/file/upload/risk_identify_operate/500/",
      uploadFileData:{
  ...
  methods: {
    edit(flag, id) {
      if (flag == "add") {
        this.titleEdit = "添加涉氨场所/环节/部位基本信息";
      } else {
        this.titleEdit = "修改涉氨场所/环节/部位基本信息";
      }
      this.isVisible = true;
      this.updateId = id;
    },
    ...
    del(id) {
      ...
      .then(() => {
        specialInfoService.deleteHById(id, res => {
          if (1 == res.code) {
            this.$message({
              type: "success",
              message: "操作成功!"
