...
//    private static String message = " 上海指旺信息科技有限公司（深圳分公司）";
    private static String centerName = "测试章";
    private static String year = "2018年06月11日";
    private static String number = "45000002267";

    /**
     * 个人签名
     * @param signature 签章名称
     * @return BufferedImage
     */
    public static BufferedImage createSignature(String signature){
        // 定义图像buffer
        BufferedImage buffImg = new BufferedImage(100, 50, BufferedImage.TYPE_INT_RGB);
        Graphics2D g = buffImg.createGraphics();
        // 增加下面代码使得背景透明
        buffImg = g.getDeviceConfiguration().createCompatibleImage(100, 50, Transparency.TRANSLUCENT);
        g.dispose();
        ...
        // 绘制圆角矩形
        g.drawRoundRect(0,0,98,45,20,20);
        // 写入签名
        g.setFont(new Font("微软雅黑", Font.LAYOUT_LEFT_TO_RIGHT, 30));
        g.drawString(signature, 5, 35);
     ...
     * @return BufferedImage
     */
    public static BufferedImage createEnterpriseSignature(String signature,String centerName){
        // 定义图像buffer
        ...
        if (StringUtils.isNotBlank(centerName)) {
            ...
            g.drawString(centerName, CENTERX - (55), CENTERY + (50 + 50));
        }
...
//        g.drawString(year, CENTERX -(60), CENTERY +(30+80));
        ...
        //根据输入字符串得到字符数组
        String[] messages2 = signature.split("",0);
    ...
    public static void main(String[] args) {
        BufferedImage image = createEnterpriseSignature(" 上海指旺信息科技有限公司","财务专用");
        try {
            String filePath = "D:/upload/signature.png";
            ImageIO.write(image, "png", new File(filePath)); //将其保存在D:\\下，得有这个目录
        } catch (Exception ex) {
            ex.printStackTrace();
        }

        BufferedImage signature = createSignature("刘长海");
        try {
            ImageIO.write(signature, "png", new File("D:/upload/lch-signature.png"));
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
